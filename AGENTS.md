...
All commits must strictly adhere to the highest standards of professional software engineering. AI agents executing git operations must follow these rules without exception:

### 1. Conventional Commits Format
Use the standard Conventional Commits specification:
`<type>[optional scope]: <description>`

- **Types:** Use `feat`, `fix`, `docs`, `style`, `refactor`, `perf`, `test`, `build`, `ci`, `chore`, or `revert`.
- **Scope:** Use a concise scope if it clarifies the module (e.g., `feat(api): ...`).

### 2. Style and Structure
- **Imperative Mood:** Write the subject line as a command (e.g., `Add user authentication`, NEVER `Added...` or `Adds...`).
- **Length Limits:** Keep the subject line under 50 characters. Do not end it with a period. If a body is necessary, separate it from the subject with a blank line and wrap text at 72 characters.
- **Context:** The body should explain *what* changed and *why*, not *how* (the code explains the how).

### 3. Strict No-Attribution Policy
- **NO AI Attributions:** Do absolutely not include tags like `Co-authored-by: AI`, `Generated by <Tool>`, or any mention that the commit was authored or assisted by an AI, LLM, or CLI agent. 
- **NO Unprofessional Phrasing:** Maintain a strictly objective and professional tone. Do not use colloquialisms, "crush" attributions, emojis, or conversational language in the commit history. The commit must look exactly as if written by a highly skilled human senior engineer.
