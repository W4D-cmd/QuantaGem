...
interface SortableSuggestionCardProps {
  suggestion: UserSuggestion;
  onDelete: (suggestion: UserSuggestion, e: React.MouseEvent) => void;
  onClick: (suggestion: UserSuggestion) => void;
}

function SortableSuggestionCard({ suggestion, onDelete, onClick }: SortableSuggestionCardProps) {
  ...
  return (
    ...
    >
      <button
        onClick={(e) => onDelete(suggestion, e)}
...
const NewChatScreen: React.FC<NewChatScreenProps> = ({
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      const newSuggestions = arrayMove(suggestions, oldIndex, newIndex);
      setSuggestions(newSuggestions);

      const orderedIds = newSuggestions.map((s) => s.id);
  ...
  return (
    <>
        ...
        >
          ...
          >
            ...
            <div className="text-left w-full">
              <span className="text-sm font-medium text-neutral-700 dark:text-zinc-400 mb-3 block">
                Suggestions
              </span>
              ...
              >
                ...
                <DragOverlay>
                  {activeSuggestion ? (
                    ...
                    >
                      <div className="flex items-center gap-3 mb-1">
                        {getIconComponent(activeSuggestion.icon)}
                        <h4 className="font-semibold text-neutral-800 dark:text-zinc-300">
      ...
      <ConfirmationModal
        ...
        message={
          <span>
            Are you sure you want to delete <strong>{suggestionToDelete?.title}</strong>? This
