...
export default function ToggleApiKeyButton({ selectedKey, onToggleAction }: Props) {
  ...
  return (
    <button
      ...
      className={`cursor-pointer h-9 flex justify-center items-center gap-2 px-4 rounded-full text-sm font-medium transition-colors
    ...
    >
      {isPaid ? (
        <SolidCurrencyDollarIcon className="size-5 text-white dark:text-neutral-900 transition-colors duration-300 ease-in-out" />
      ) : (
        <OutlineCurrencyDollarIcon className="size-5 text-neutral-500 dark:text-neutral-300 transition-colors duration-300 ease-in-out" />
      )}
